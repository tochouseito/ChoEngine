...
Quaternion Theatria::Math::FromMatrix(const float4x4& m) noexcept
{
	// 1) 3x3 の取り出し
...
[[nodiscard]]
float3 Theatria::Math::TransformPoint(const float3& v, const float4x4& M) noexcept
{
    float3 r;
...
[[nodiscard]]
float3 Theatria::Math::TransformVector(const float3& v, const float4x4& M) noexcept
{
...
[[nodiscard]]
float4x4 Theatria::Math::TranslateMatrix(const float3& t) noexcept
{
...
[[nodiscard]]
float4x4 Theatria::Math::ScaleMatrix(float s) noexcept
{
    float4x4 m = float4x4::Identity();
    m.m[0][0] = s;
    m.m[1][1] = s;
...
[[nodiscard]]
float4x4 Theatria::Math::ScaleMatrix(const float3& s) noexcept
{
    float4x4 m = float4x4::Identity();
    m.m[0][0] = s.x;
    m.m[1][1] = s.y;
...
[[nodiscard]]
float4x4 Theatria::Math::ScaleMatrix(const Scale& s) noexcept
{
    float4x4 m = float4x4::Identity();
    m.m[0][0] = s.x;
    m.m[1][1] = s.y;
...
[[nodiscard]]
float4x4 Theatria::Math::XAxisMatrix(float radian) noexcept
{
    float4x4 m = float4x4::Identity();
...
[[nodiscard]]
float4x4 Theatria::Math::YAxisMatrix(float radian) noexcept
{
    float4x4 m = float4x4::Identity();
    const float c = std::cos(radian);
    const float s = std::sin(radian);
...
[[nodiscard]]
float4x4 Theatria::Math::ZAxisMatrix(float radian) noexcept
{
    float4x4 m = float4x4::Identity();
    const float c = std::cos(radian);
    const float s = std::sin(radian);
...
[[nodiscard]]
float4x4 Theatria::Math::RotateXYZMatrix(const float3& r) noexcept
{
    return XAxisMatrix(r.x) * YAxisMatrix(r.y) * ZAxisMatrix(r.z);
}

[[nodiscard]]
float4x4 Theatria::Math::ViewportMatrix(float left, float top, float width, float height, float minDepth, float maxDepth) noexcept
{
    float4x4 m = float4x4::Identity();
    m.m[0][0] = width / 2.0f;
...
[[nodiscard]]
float4x4 Theatria::Math::PerspectiveFovMatrix(float fovY, float aspectRatio, float nearClip, float farClip) noexcept
{
...
[[nodiscard]]
float4x4 Theatria::Math::OrthographicMatrix(float left, float top, float right, float bottom, float nearClip, float farClip) noexcept
{
    float4x4 m = float4x4::Identity();
...
[[nodiscard]]
float Theatria::Math::Normalize(float x, float min, float max) noexcept
{
    if (max - min == 0.0f)
    {
        return 0.0f; // ゼロ除算回避
...
float4x4 Theatria::Math::MakeRotateAxisAngle(const float3& axis, float angle)
{
...
float4x4 Theatria::Math::DirectionToDirection(const float3& from, const float3& to)
{
    // 入力ベクトルを正規化
...
float Theatria::Math::DegreesToRadians(float degrees)
{
    return degrees * PI / 180.0f;
}

float3 Theatria::Math::DegreesToRadians(const float3& degrees)
{
    return float3(
        degrees.x * PI / 180.0f,
        degrees.y * PI / 180.0f,
...
float Theatria::Math::RadiansToDegrees(float radians)
{
    return radians * 180.0f / PI;
}

float3 Theatria::Math::RadiansToDegrees(const float3& radians)
{
    return float3(
        radians.x * 180.0f / PI,
        radians.y * 180.0f / PI,
...
Quaternion Theatria::Math::MakeRotateAxisAngleQuaternion(const float3& axis, float angle)
{
    float3 normAxis = axis;
...
float3 Theatria::Math::RotateVector(const float3& vector, const Quaternion& quaternion)
{
    // q * v * q^-1 を計算する
...
float4x4 Theatria::Math::MakeRotateMatrix(const Quaternion& quaternion)
{
    float4x4 matrix;
...
Quaternion Theatria::Math::FromEulerAngles(const float3& eulerAngles)
{
...
float3 Theatria::Math::ToEulerAngles(const Quaternion& q, RotationOrder order)
{
    float3 angles;
...
float4x4 Theatria::Math::MakeAffineMatrix(const float3& scale, const float3& rotate, const float3& translate)
{
    float4x4 result;
...
float4x4 Theatria::Math::MakeAffineMatrix(const Scale& scale, const Quaternion& rotate, const float3& translate)
{
    float4x4 result;
    float4x4 scaleMatrix = ScaleMatrix(scale);
...
float3 Theatria::Math::TransformDirection(const float3& v, const float4x4& m)
{
    return {
...
float Theatria::Math::LerpShortAngle(float startAngle, float endAngle, float t)
{
    // 角度の差を計算
...
Quaternion Theatria::Math::MakeLookRotation(const float3& forward, const float3& up)
{
    float3 f = forward;
    f.Normalize();
...
float3 Theatria::Math::GetForwardVectorFromMatrix(const float4x4& rotMatrix)
{
    return float3(
        rotMatrix.m[0][2], // x
        rotMatrix.m[1][2], // y
...
SRT Theatria::Math::DecomposeMatrix(const float4x4& in)
{
    SRT out;
...
Quaternion Theatria::Math::MakeQuaternionRotation(const float3& rad, const float3& preRad, const Quaternion& quaternion)
{
    // 差分計算
...
Quaternion Theatria::Math::MakeEulerRotation(const float3& rad)
{
    // オイラー角からクォータニオンを作成
    // 各軸のクオータニオンを作成
...
float4x4 Theatria::Math::BillboardMatrix(const float4x4 cameraMatrix)
{
    float4x4 result;
...
float2 Theatria::Math::WorldToScreen(const float3& worldPos, const float4x4& viewMatrix, const float4x4& projMatrix, uint32_t screenWidth, uint32_t screenHeight)
{
    // Viewport行列
...
float Theatria::Math::Lerp(float start, float end, float t)
{
    return start + (end - start) * t;
}

// 移動成分のみ抽出
float3 Theatria::Math::GetTranslation(const float4x4& m)
{
    return float3(m.m[3][0], m.m[3][1], m.m[3][2]);
}

// 球面線形補間 (Slerp) 関数
float3 Theatria::Math::Slerp(const float3& v1, const float3& v2, float t)
{
    // 正規化
    float3 start = v1;
    float3 end = v2;
...
// 線形（普通のLerpと同じ）
// 線形（普通のLerpと同じ）
float Theatria::Math::easing::Linear(float t)
{
    return t;
}
...
// 二乗で加速（最初ゆっくり → 後半速い）
// 二乗で加速（最初ゆっくり → 後半速い）
float Theatria::Math::easing::EaseInQuad(float t)
{
    return t * t;
}
...
// 二乗で減速（最初速い → 後半ゆっくり）
// 二乗で減速（最初速い → 後半ゆっくり）
float Theatria::Math::easing::EaseOutQuad(float t)
{
...
// 二乗で加減速（最初ゆっくり → 中盤速い → 最後ゆっくり）
float Theatria::Math::easing::EaseInOutQuad(float t)
{

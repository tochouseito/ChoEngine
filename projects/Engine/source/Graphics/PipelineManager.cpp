...
[[nodiscard]] bool Theatria::Graphics::PipelineManager::Initialize(ID3D12Device* device, ShaderCompiler* compiler, DescriptorAllocator* descriptorAllocator)
{
    m_pDescriptorAllocator = descriptorAllocator;

    Theatria::Core::Parser::SaveGraphicsPipelines_ini();
    Theatria::Core::Parser::SaveComputePipelines_ini();
    // デフォルトパイプラインの生成
    CreateDefaultPipelines(device, compiler);
...
void Theatria::Graphics::PipelineManager::CreateDefaultPipelines(ID3D12Device* device, ShaderCompiler* compiler)
{
    ...
    // テスト
    {
        ...
        range.NumDescriptors = 3; // t0, t1, t2
    ...
    {
        // Compute
        m_ComputePipelines = Theatria::Core::Parser::LoadComputePipelines_ini();
        ...
        csDesc.entryPoint = L"CSMain";
        csDesc.profile = L"cs_" + Graphics::ShaderProfileToWString(Config::Graphics::HighestShaderModel);
        csDesc.filePath = Utility::ToUTF16(Config::FilePath::ShaderDirectory + "Basic.cs.hlsl");
        ComPtr<IDxcBlob> csBlob = compiler->GetOrCompileShader(csDesc);

        // RootSignature
        D3D12_ROOT_PARAMETER csParams[3] = {};

        // 0: SRV テーブル
        D3D12_DESCRIPTOR_RANGE srvRange = {};
        srvRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
        srvRange.BaseShaderRegister = 0;
        srvRange.NumDescriptors = 3; // Object, Transform, ModelData
        ...
        csParams[0].DescriptorTable.pDescriptorRanges = &srvRange;

        // 1: UAV テーブル
        D3D12_DESCRIPTOR_RANGE uavRange = {};
        uavRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
        uavRange.BaseShaderRegister = 0;
        uavRange.NumDescriptors = 2; // IndirectCommand, Count
        ...
        csParams[1].DescriptorTable.pDescriptorRanges = &uavRange;

        // 2: objectCount
        csParams[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;
        csParams[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
        csParams[2].Constants.ShaderRegister = 0; // b0
        csParams[2].Constants.Num32BitValues = 1;

        D3D12_ROOT_SIGNATURE_DESC crootSigDesc = {};
        crootSigDesc.NumParameters = _countof(csParams);
        crootSigDesc.pParameters = csParams;
        crootSigDesc.NumStaticSamplers = 0;
        crootSigDesc.pStaticSamplers = nullptr;
        crootSigDesc.Flags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT;
        // シリアライズ
        ComPtr<ID3DBlob> serializedRootSig = nullptr;
        ComPtr<ID3DBlob> errorBlob = nullptr;
        HRESULT hr = D3D12SerializeRootSignature(
...
void Theatria::Graphics::PipelineManager::CreateGraphicsPipeline(ID3D12Device* device, GraphicsPipelineSettings& setting, ShaderCompiler* compiler)
{
    ...
    setting.argsDescriptorTableID = m_pDescriptorAllocator->Allocate(DescriptorAllocator::TableKind::Buffers);
    m_pDescriptorAllocator->CreateUAVBuffer(
        setting.argsDescriptorTableID,
        &setting.argsBuffer);

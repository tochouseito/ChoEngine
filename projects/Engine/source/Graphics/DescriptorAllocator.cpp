...
[[nodiscard]]
bool DescriptorAllocator::Initialize(RenderDevice* pRenderDevice,
    uint32_t texCap,
    uint32_t bufCap,
    uint32_t rtCap,
    uint32_t dsCap)
        ...
        case HeapType::CBV_SRV_UAV:
        {
            ...
            Core::LogAssert::Verify(
                m_pRenderDevice->m_Device->CreateDescriptorHeap(
                    &desc, IID_PPV_ARGS(&m_DescriptorHeaps[i])),
                "DescriptorAllocator", "Failed CreateDescriptorHeap");
            ...
            Core::LogAssert::Verify(
                m_pRenderDevice->m_Device->CreateDescriptorHeap(
                    &desc, IID_PPV_ARGS(&m_GpuSUVHeap)),
                "DescriptorAllocator", "Failed CreateDescriptorHeap");
            SetD3D12Name(m_GpuSUVHeap.Get());
...
void Theatria::Graphics::DescriptorAllocator::CreateDSV(TableID& id, GpuResource* res)
{
    // DSVの設定
    D3D12_DEPTH_STENCIL_VIEW_DESC dsvDesc = {};
    dsvDesc.Format = Config::Graphics::DefaultDepthDXGIFormat;
    dsvDesc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
    dsvDesc.Flags = D3D12_DSV_FLAG_NONE;
    m_pRenderDevice->m_Device->CreateDepthStencilView(res->GetResource(), &dsvDesc, GetCPUHandle(id));
}
...
void DescriptorAllocator::CopyToGpuHeap(const TableID& id)
{
    ...
    Core::LogAssert::Check(m_GpuSUVHeap != nullptr,
    ...
    m_pRenderDevice->m_Device->CopyDescriptorsSimple(
        1,
        dst, // dest (GPU ヒープ)
        src, // src  (CPU ヒープ)
